...
function renderSummary(data) {
  ...
  const cards = [
    ...
    },
    {
      ...
      suffix: '亿',
      note: `面积口径：${state.area === 'amount' ? '成交额' : state.area === 'marketCap' ? '总市值' : '资金规模'}`,
    },
    {
      label: '主力净流入',
    ...
    },
    {
      ...
      suffix: '%',
      note: strongest ? `最热：${strongest.name}` : '暂无数据',
      valueClass: classByValue(avgChange),
    },
  ];
...
function renderDetail(item) {
  ...
  els.detailPanel.innerHTML = `
    ...
    <section class="detail-section">
      <h3>领涨与资金核心</h3>
      <div class="metric-list" style="padding:0; grid-template-columns:1fr;">
    ...
    <section class="detail-section">
      <h3>相关 ETF / 基金入口</h3>
      <div class="etf-list">${item.relatedEtfs.map((etf) => `<span class="etf-pill">${etf}</span>`).join('')}</div>
    </section>
  `;
}
